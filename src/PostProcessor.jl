struct PostProcessor{
    ExoPP <: FiniteElementContainers.PostProcessor,
    MO, # TODO type to material output
    MV
}
    exodus_pp::ExoPP
    mat_outputs::MO
    mat_vars::MV
end

function PostProcessor(sim)
    mesh = UnstructuredMesh(sim.mesh_file)
    V_q = FunctionSpace(mesh, L2Field, Lagrange)
    disp_var = assembler(sim.objective).dof.var

    mat_outputs, mat_vars = create_material_output(sim.objective, V_q, StandardMaterialOutput{Float64})
    update_material_output!(mat_outputs, sim.objective, sim.u, sim.p)

    exodus_pp = FiniteElementContainers.PostProcessor(
        mesh, sim.output_file, 
        disp_var, mat_vars...
    )
    pp = PostProcessor(exodus_pp, mat_outputs, mat_vars)
    _post_process_common!(pp, sim, 1)

    return pp
end

function close(pp::PostProcessor)
    FiniteElementContainers.close(pp.exodus_pp)
end

function post_process(pp::PostProcessor, sim, n)
    _post_process_common!(pp, sim, n)
end

function _post_process_common!(pp, sim, n)
    mat_outputs, mat_vars = pp.mat_outputs, pp.mat_vars

    U_names = names(assembler(sim.objective).dof.var)
    # U = objective_cache.solution
    # p = objective_cache.parameters
    # U = p.h1_field
    # update_field_dirichlet_bcs!(p.field, p.dirichlet_bcs)
    # update_field_unknowns!(p.field, assembler(objective_cache), U)
    update_material_output!(mat_outputs, sim.objective, sim.u, sim.p)

    write_times(pp.exodus_pp, n, sum(sim.p.times.time_current))
    write_field(pp.exodus_pp, n, U_names, sim.p.field)
    # write_field(pp, n, )

    # for (block, val) in pairs(mat_outputs)
    #     write_field(pp.exodus_pp, n, String(block), "algorithmic_energy", val.algorithmic_energy)
    #     write_field(pp.exodus_pp, n, String(block), "cauchy_stress", val.cauchy_stress)
    #     write_field(pp.exodus_pp, n, String(block), "displacement_gradient", val.displacement_gradient)
    # end
end
